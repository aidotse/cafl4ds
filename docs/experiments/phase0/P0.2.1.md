# Positive control — the healthy SimSiam baseline (P0.2.1)

This Phase-0 sub-study (P0.2.1; see the [Phase 0 overview](index.md)) **continues the collapse-instrument calibration**
begun in [P0.2.0](P0.2.0.md): same vehicle (predictorless / stop-gradient-off SimSiam as the forced-collapse arm), same
goal (prove the instruments catch a real collapse), but it fixes the flaw P0.2 exposed — the intact "healthy" arm was
**not actually a healthy baseline**, so the gate had no trustworthy upper reference. Here we look for this baseline,
then ask how far that baseline reaches: a [2×2 regime matrix](#does-the-baseline-generalize-the-22-regime-matrix) —
{IID, class-blocked} × {40 epochs, single pass} (+ ablations on LR and horizon length) — testing whether the calibration
...
P0.2 ran both arms **from-scratch, single-pass (~25 steps), on the class-blocked stream** — the P0.1.0 toy horizon. At
...
— is the **same tiny ViT** as P0.1.0/P0.2 (`embed_dim=96`, `depth=4`, `img_size=32`). What changed, and why each matters
...
arms, **IID** STL-10, full train split, `batch_size=128`, 40 epochs, warmup+cosine LR, tiny ViT (same backbone as
P0.1.0). `pc_*` = collapse ablation, `hc_*` = intact SimSiam. Both seeds **PASS** (later corroborated at **4 seeds** by
the [horizon sweep](#how-cheap-can-the-reference-be-a-horizon-sweep-is-40-the-minimum-and-is-it-a-plateau),
worst-of-four 2.47×).
...
Per the P0.1.0/P0.2 milestone convention, the identical run (`device=hpu`, inside the container) reproduces on a Gaudi 2
...
1. **Mechanism, resolved on the horizon axis.** The healthy re-expansion **saturates by ~40 epochs at the *verdict*
    level** — the worst-of-seeds ≥ 2× holds from 40 on — but the *magnitude* is not a hard plateau: it keeps creeping
    (5.53 at ep40 → 5.79 at ep80 here). An independent re-run of this sweep in [P0.2.2 E2](P0.2.2.md#e2--horizon-sweep)
    puts healthy@bb at 5.46 → 6.13 over the same ep40 → 80 (a +12 % climb), so the two horizon sweeps agree on the
    *verdict* (monotone ratio, robust ≥ 2× from ~25–40 ep) but **not** on the magnitude — reinforcing the device/thread
    caveat that only the verdict ports, not the number. Above 40 the ratio still creeps up almost entirely because the
    **PC keeps slowly sinking** (1.94 → 1.77 at 80, *below* the nominal ~1.9 BN floor; P0.2.2 E2 corroborates, 2.69 →
    1.67). "Healthy" is a horizon-saturating quantity; "collapsed" is a slowly-*deepening* one — which is also
    (partial) evidence RankMe reads collapse **depth** monotonically on the deep-collapse side, a down-payment on the
    incipient-collapse gap the audit flags ([P0.2 audit](../audits/P0.2.md), C1). The dip bottom itself drifts *later*
    with horizon (epoch ~3 at 5ep → ~8–9 at 20ep) because the recovery is anneal-phased. **SimSiam at this toy scale
    needs ~40 epochs for the healthy signal to saturate, and the contrast becomes trustworthy only once it does.**
...
P0.2.2's [`E1/iid_40ep_seed*`](artifacts/P0.2.2/E1/) (same config, RankMe re-read — P0.2.2 cross-checks the reference
did not move), so it is re-backed there. Its LR / horizon *ablation* runs were exploratory and not preserved as clean
per-run artifacts; those tables are reproducible via the P0.2.1 "How to run" commands.
...
# image, so use the container's system python, and bind STL-10:
