...
calibration** begun in [P0.2.0](P0.2.0.md): same vehicle (predictorless / stop-gradient-off SimSiam as the
forced-collapse arm), same goal (prove the instruments catch a real collapse), but it fixes the flaw P0.2 exposed — the
intact "healthy" arm was **not actually a healthy baseline**, so the gate had no trustworthy upper reference. Here we
look for this baseline, then ask how far that baseline reaches: a
[2×2 regime matrix](#does-the-baseline-generalize-the-22-regime-matrix) — {IID, class-blocked} × {40 epochs, single
pass} (+ ablations on LR and horizon length) — testing whether the calibration survives toward the project's target
regime (correlated ordering, single-pass streaming). It does not: only the clean, long-horizon corner certifies.
