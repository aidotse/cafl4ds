...
calibration** begun in [P0.2](P0.2.md): same vehicle (predictorless / stop-gradient-off SimSiam as the forced-collapse
arm), same goal (prove the instruments catch a real collapse), but it fixes the flaw P0.2 exposed — the intact "healthy"
arm was **not actually a healthy baseline**, so the gate had no trustworthy upper reference. Here we look for this
baseline, then ask how far that baseline reaches: a
