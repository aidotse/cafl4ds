...
The first Phase-0 sub-study (see the [Phase 0 overview](index.md)). The streaming SSL adaptation loop, the class-blocked
STL-10 stream, both SSL backbones, and separate loss/health logging exercise the full `C × I` matrix under the B-floor
knob. The reference numbers below were computed on **CPU**, and the same matrix has since been reproduced on a **Gaudi 2
HPU** as an infrastructure milestone. Additionally, this is an initial investigation on collapse and forgetting
calibration and match to a good baseline for both.

## Scope of this slice

On top of the already-verified instruments (`cafl4ds/measurements.py`) we add the machinery to drive those instruments
over a live, correlated stream:
...
- **Positive controls + the Phase-0 gate.** The plan's formal Phase-0 exit is **per-mode calibration** — each failure
    mode's PC must fire *and* a healthy baseline stay quiet (collapse, forgetting, instability). The instruments are
    validated (`tests/unit/test_measurements.py`) and now demonstrably *drivable* over a stream, but no positive control
    is built here. The **collapse** PC is the natural next increment (now done —
    [P0.2.0](P0.2.0.md)/[P0.2.1](P0.2.1.md)); the **forgetting** and **instability** PCs follow.
...
point it at STL-10 (which lives outside the mounted repo) with the `-m` flag:
