...
| P0.2.4 | [Positive control (collapse gate) - false-positive stress on the quiet side](P0.2.4.md) | Motivation: every P0.2.x run so far varies how the PC and healthy arms *separate* — the healthy arm is always a well-behaved SimSiam — so the suite's **false-positive rate** is untested. Promotes P0.2.2's flagged E6 follow-up: push a *genuinely-healthy* representation into regimes that superficially resemble collapse (low-diversity data, heavy augmentation, deliberate undertraining) and confirm no instrument false-fires — the failure mode that most hurts the loop (a false fire mis-aims selection). | ✅ **Complete** (found FP boundaries: `offdiag_cov` @ backbone false-fires under undertraining/low-diversity → read it at the projector only; suite not FP-free at toy scale) |
...
- [`artifacts/P0.2.4/`](artifacts/P0.2.4/) — the 12 false-positive-stress runs (`<stressor>/seed_N.json` for
    `low_diversity`, `heavy_aug`, `undertrained`); the quiet-verdict + envelope behind the FP-boundary map.
