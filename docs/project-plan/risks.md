...
| -- | -- | -- |
| **False negative on degradation** (underpowered demo) — collapse *or* forgetting | No degradation seen under stress | Pressure-sweep incl. from-scratch + no-replay + long horizon + adverse LR schedule; **prior art warns** MAE resists & replay trivially protects (continual-MAE pretraining), so read the demo as **envelope + selection lever**, not a spontaneous crater; collapse on joint-embedding, forgetting on MAE; for collapse **control batch-size + model-size** (both induce dimensional collapse independent of the diet — Jing 2022; SimSiam size-sensitivity); **PC must fire** first |
...
| **Synthetic-correlation artifact** (STL-10) | Degradation on 1a pilot but not 1b BDD | STL-10 validates the apparatus only; the claim rests on BDD's real correlation; non-transfer means class-blocking drove it — say so, lean on BDD; also a rank drop on STL-10 (few classes, small scale) may be **benign tunnel-effect compression**, not degradation (Masarczyk 2023) — another reason it is smoke-test only |
| Filter wins on collapse metric but not downstream | RankMe up, probe flat | Optimized the thermometer, not the patient — validate on downstream + forgetting; RankMe is **necessary-not-sufficient** (Garrido 2022), so read a metric **suite** (add intrinsic dimension — most reliable label-free downstream proxy, Arputharaj et al. TMLR 2026), not one needle |
...
| Adaptation doesn't beat frozen (RanDumb) | B5 ≥ adapted | Existential + **contested**: RanDumb (NeurIPS 2024) shows random *outperforms* online CL representations (discriminative; open for SSL), but Memory Storyboard shows streaming-SSL adaptation *can* win — test head-on in P1.2; if B5 ≥ adapted, reframe to selection-for-efficiency |
...
| Coupling result just re-confirms prior (DiSF/CCS offline; continual-MAE replay) | No surprise | Hunt loop instability / flip inversion / closed-loop benefit explicitly; for forgetting the novelty is **selection as the lever** + live coupling, not replay-vs-none (continual-MAE pretraining already settled that) |
